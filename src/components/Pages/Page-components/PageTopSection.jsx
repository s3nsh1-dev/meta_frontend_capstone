...
const PageTopSection = ({
  ...
  const toRenderSubText = () => {
    switch (type_for_sub_text) {
      case "p":
        return <p className="text-white">{sub_text}</p>;
      case "h3":
        return <h3 className="text-white">{sub_text}</h3>;
      default:
        return null;
    }
  };
      ...
      <div>
        <img
          className={`page-main-image top-section-page-image-margin ${
            image_position === "left"
              ? "top-section-page-image-no-margin"
              : null
          }`}
          src={image}
          alt="image_not_found"
  ...
  return (
    <>
      <section
        className={`section page-top-section-left 
