    ...
    const checkAvailability = () => {
      for (const element of ProfileForUser[type]) {
        if (element["id"] === item["id"]) {
          console.log(element);
          const tempQty =
            parseFloat(element["dish-quantity"]) +
            parseFloat(item["dish-quantity"]);
          item["dish-quantity"] = tempQty + "";
          const typeArray = ProfileForUser[type].filter(
            (ele) => ele["id"] !== item["id"]
          );
    ...
    return (
      <article className="order-card">
        ...
        <div className="order-card-item-details">
          ...
          <div>
            <p className="paragraphText DishText">{dish["dish-details"]}</p>
          </div>
          <div className="order-card-qty-price">
            ...
            <span className="order-card-price">
              $ {parseFloat(dish["dish-price"] * itemQty).toFixed(2)}
            </span>
          </div>
        </div>
        {dishQty === undefined ? (
          ...
          <div className="order-card-btn-box-profile">
            <button
              className="order-card-btn order-card-btn-order-profile"
              onClick={() => {
