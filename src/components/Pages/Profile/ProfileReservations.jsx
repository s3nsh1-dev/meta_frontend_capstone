...
import SetDataContext from "../../../data/SetDataContext";
...
const ProfileReservations = () => {
  ...
  const { ReturnedProfile, HandleUpdateReservations } =
  ...
  const handleDeleteReservation = (date, slotTime) => {
    ...
    const garbReservationByTime = garbReservationByDate["timeSlots"].find(
      (ele) => ele["slotTime"] === slotTime
    );
    const reservationByTimeIndex = garbReservationByDate["timeSlots"].indexOf(
      garbReservationByTime
    );

    const updatedBookings = reservations;
    updatedBookings["bookings"][reservationByDateIndex]["timeSlots"].splice(
      reservationByTimeIndex,
      1
    );
        ...
        <div className="profile-item-card-container">
          <div className="profile_reservation_card_parts">
            <div className="profile_reservation_text_box">
              ...
              <span className="profile_reservation_sub_text">
                {item.occasion}
              </span>
            </div>
            <div className="profile_reservation_text_box">
  ...
  const renderReservations = () => {
    ...
    return returnReservations().length !== 0 ? (
      <div className="profile-item-card-list">
        {profileReservations.map((item, index) => (
          <div key={index}>{profileReservationCard(item)}</div>
        ))}
      </div>
    ) : (
      <ShowEmptyForProfile viewOption={"reservations"} />
    );
  };

  return (
    <>
      {emailToFind !== undefined && bookings !== undefined
        ? renderReservations()
        : null}
    </>
  );
